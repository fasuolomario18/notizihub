...
function slugify(text) {
  let slug = text.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-');
...
async function salvaMarkdown(nicchia, spunto, contenuto, lingua) {
  ...
  // Fallback: usa il TLDR o il primo paragrafo, mai il titolo RSS grezzo
  const tldrMatch = contenuto.match(/<!--\s*TLDR\s*-->([\s\S]*?)<!--\s*\/TLDR\s*-->/);
  const tldrText = tldrMatch ? tldrMatch[1].trim().replace(/\n+/g, ' ') : null;
  const firstParagraph = testo.replace(/^#.+\n+/m, '').replace(/^#+.+\n+/gm, '').replace(/\*\*/g, '').split('\n').find(l => l.trim().length > 40) || '';
  const fallbackDesc = (tldrText || firstParagraph).substring(0, 155);
  const metaDesc = metaMatch ? metaMatch[1].trim() : fallbackDesc;
